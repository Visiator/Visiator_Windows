...
void load_fonts() {
	...
	HRSRC hRes_font3 = FindResource(hMod, MAKEINTRESOURCE(20002), L"BINARY");
	HGLOBAL hGlob_font3 = LoadResource(hMod, hRes_font3);
	BYTE *lpbArray_font3 = (BYTE*)LockResource(hGlob_font3);
	DWORD sz_font3;
	sz_font3 = SizeofResource(hMod, hRes_font3);

	font[3]->load_dummy(lpbArray_font3, sz_font3);

	//////////////////////////////////////////////////////////////////////

	font[4] = new FONT();

	HRSRC hRes_font4 = FindResource(hMod, MAKEINTRESOURCE(20002), L"BINARY");
	HGLOBAL hGlob_font4 = LoadResource(hMod, hRes_font4);
	BYTE *lpbArray_font4 = (BYTE*)LockResource(hGlob_font4);
	DWORD sz_font4;
	sz_font4 = SizeofResource(hMod, hRes_font4);

	font[4]->load_dummy(lpbArray_font2, sz_font4);
}
...
bool get_screenshot(MULTIDISPLAY *multidisplay, SCREEN_LIGHT_one_byte *screen, SCREEN_LIGHT_12bit *screen_12bit) {
	...
	void *BitBlt_raw_buffer;
	BitBlt_raw_buffer = nullptr;
	...
	if (screen_12bit != nullptr) {
		if (screen_12bit->w != g_nWidth || screen_12bit->h != g_nHeight) {
			screen_12bit->set_new_size_(g_nWidth, g_nHeight);
		}
	}
...
void RUN_VIEWER_EPASS(wchar_t *exe_file_name, wchar_t *ID, wchar_t *PASS, unsigned char *PASS_ENCR_16byte) {
	...
	//WideCharToMultiByte(CP_ACP, 0, ID, -1, id, 50, NULL, NULL); // TODO 2019+ проверить как в других языках конверитуется из wchar_t
	//WideCharToMultiByte(CP_ACP, 0, PASS, -1, pass, 50, NULL, NULL); // TODO 2019+ проверить как в других языках конверитуется из wchar_t
