...
int SERVICE___STOP(wchar_t *service_name) {
	...
	if (r == 0) {
		err = GetLastError();
		if (err == ERROR_ACCESS_DENIED)              { sudp("ERROR_ACCESS_DENIED");	}; //	Дескриптор не имеет необходимого права доступа.
		if (err == ERROR_DEPENDENT_SERVICES_RUNNING) { sudp("ERROR_DEPENDENT_SERVICES_RUNNING"); }; //		Служба не может быть остановлена, потому что другие запущенные службы зависят от нее.
		if (err == ERROR_INVALID_HANDLE)             { sudp("ERROR_INVALID_HANDLE"); }; //		Указанный дескриптор не был получен, при помощи использования функции CreateService или OpenService, или дескриптор больше не действителен.
		if (err == ERROR_INVALID_PARAMETER)          { sudp("ERROR_INVALID_PARAMETER"); }; //		Требуемый управляющий код не определен.
		if (err == ERROR_INVALID_SERVICE_CONTROL)    { sudp("ERROR_INVALID_SERVICE_CONTROL"); }; //		Требуемый управляющий код не действителен, или он неприемлемый для службы.
		if (err == ERROR_SERVICE_CANNOT_ACCEPT_CTRL) { sudp("ERROR_SERVICE_CANNOT_ACCEPT_CTRL"); }; //		Требуемый управляющий код нельзя отправить службе, потому что состояние службы - SERVICE_STOPPED, SERVICE_START_PENDING или SERVICE_STOP_PENDING.
		if (err == ERROR_SERVICE_NOT_ACTIVE)         { sudp("ERROR_SERVICE_NOT_ACTIVE"); }; //		Служба не запустилась.
		if (err == ERROR_SERVICE_REQUEST_TIMEOUT)    { sudp("ERROR_SERVICE_REQUEST_TIMEOUT"); }; //		Был запущен процесс для службы, но он не вызывал функцию StartServiceCtrlDispatcher, или поток, который вызвал StartServiceCtrlDispatcher может быть блокирован функцией обрабатывающей программы управления.
